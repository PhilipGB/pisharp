...
- PiSharp started from clean `main` at `1520f311196fdbd6a3f2eae07f09bc3f80e55353`. The latest successful Linux CI run is [run 36074937544](https://github.com/PhilipGB/pisharp/actions/runs/36074937544), for `51ec21b1b2337dc90c4d61b5bb2e3abdcdc86f6d` on 2026-09-24; it covers outside-Git `.ignore`/`.fdignore` handling. Git-path ignore support is validated locally and awaiting commit.
...
1. **Coding tools (active):** complete `read`, `bash`, `edit`, `write`, `grep`, `find`, and `ls` contracts using upstream implementation/tests, deterministic local cases, pinned differentials, and real agent turns. Once major behavior is covered, move on rather than polishing obscure edge cases.
   ...
   - `grep`, `find`, `ls`: the managed fallback reads `.gitignore`/`.ignore` outside Git, plus `.rgignore` for grep and `.fdignore` for find. Inside Git it applies those same per-tool rules to Git-listed files, including staged ignored paths and rules inherited from repository ancestors. Fixtures cover nested rules and tool-specific behavior; `find` includes matching empty directories in and outside Git. Grep matches Pi's 50 KiB head truncation, line marker and separate byte/match notices. SearchToolsTests pass 10/10; the full Release suite passes 325/325 with 0 skipped, format passes, and the warnings-as-errors build has 0 warnings/errors. Still open: global exclude files, complete fd/rg matching/traversal parity, permission and symlink differentials, and structured renderer details.
...
Commit and push Git-path per-tool ignore handling, then continue global exclude and fd/rg filesystem differentials. Return to write error mapping and Bash direct RPC/result differentials during the broader parity pass.
