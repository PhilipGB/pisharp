...
- Added Pi-style request-wide model pricing tiers to catalog/model cost metadata and usage accounting. The highest threshold strictly below input-token count applies rates to the full request; local catalog parsing and cost arithmetic tests cover below/above threshold behavior. Cache-write pricing remains unsupported because usage data does not yet expose it.
