...
1. **Coding tools (active):** complete `read`, `bash`, `edit`, `write`, `grep`, `find`, and `ls` contracts using upstream implementation/tests, deterministic local cases, pinned differentials, and real agent turns. Once major behavior is covered, move on rather than polishing obscure edge cases.
   ...
   - `grep`, `find`, `ls`: compare remaining matching, encoding, symlink, permission, glob/ignore, truncation, ordering, cancellation and result details against Pi. Existing local coverage is narrow.
...
Continue the write contract audit, including error mapping and filesystem behavior, then move through grep/find/ls. Return to Bash for direct RPC wire and pinned result/error differentials during the broader parity pass.
