...
- PiSharp started from clean `main` at `1520f311196fdbd6a3f2eae07f09bc3f80e55353`. The latest successful Linux CI run is [run 36074306953](https://github.com/PhilipGB/pisharp/actions/runs/36074306953), for `21fc49618f352bc50ac8caa3bbff88c6fbb3fd71` on 2026-09-24; it covers empty-directory `find` support. The current outside-Git `.ignore`/`.fdignore` extension is validated locally and awaiting commit.
...
1. **Coding tools (active):** complete `read`, `bash`, `edit`, `write`, `grep`, `find`, and `ls` contracts using upstream implementation/tests, deterministic local cases, pinned differentials, and real agent turns. Once major behavior is covered, move on rather than polishing obscure edge cases.
   ...
   - `grep`, `find`, `ls`: the managed fallback reads root and nested `.gitignore`/`.ignore` rules outside Git; find also reads `.fdignore`. Fixtures cover wildcard/negation and the tool-specific ignore-file distinction; `find` includes matching empty directories in and outside Git. Grep matches Pi's 50 KiB head truncation, line marker and separate byte/match notices. SearchToolsTests pass 8/8; the full Release suite passes 323/323 with 0 skipped, format passes, and the warnings-as-errors build has 0 warnings/errors. Still open: applying `.ignore`/`.fdignore` to Git-listed files, global excludes, fd/rg matching/traversal, permission and symlink differentials, and structured renderer details.
...
Commit and push the outside-Git `.ignore`/`.fdignore` support, then continue the Git-path ignore and fd/rg filesystem differential pass. Return to write error mapping and Bash direct RPC/result differentials during the broader parity pass.
