...
- Added an OpenRouter OpenAI-compatible built-in provider profile with its own endpoint, `OPENROUTER_API_KEY`, default model override, and explicit regression coverage preventing accidental use of `OPENAI_API_KEY`. This is one adapter, not broad provider/API parity; format, warnings-as-errors build, and 132 tests pass.
