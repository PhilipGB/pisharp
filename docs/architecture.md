...
4. Experimental Core Pi v3 codec, context projection and private JSONL file store are tested only on small pinned branch fixtures. Expand v1/v2 migration evidence and complete the experimental branch-to-MAF restore seam (system patches, multimodal, provider signatures, error results, round-trip capture and journal append), then replace preliminary MAF-only snapshots in **all** CLI modes. The codec is not yet the CLI's authoritative history.
