...
| Resources | `-e/--extension`, `-ne/--no-extensions`, `--skill`, `-ns/--no-skills`, `--prompt-template`, `-np/--no-prompt-templates`, `--theme`, `--use-theme`, `--no-themes`, `-nc/--no-context-files` | Local paths, trust, discovery and explicit override precedence | In progress: `-nc/--no-context-files` skips AGENTS/CLAUDE and `-ne/--no-extensions` skips discovered user/trusted-project DLLs (PTY process fixture); remaining listed flags absent |
| Runtime | `--system-prompt`, `--append-system-prompt`, `--tui-mode`, `--verbose`, `-a/--approve`, `-na/--no-approve`, `--offline` | Per-invocation overrides, trust for project resources, no accidental network activity | `--approve` and `--no-approve` gate project system prompts; `--system-prompt` and repeatable `--append-system-prompt` resolve bounded UTF-8 files/literals and override discovery (local + HTTP process fixtures); `--no-context-files` skips AGENTS/CLAUDE loading (process fixture); remaining flags absent |
