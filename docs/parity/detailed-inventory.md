...
| Resources | `-e/--extension`, `-ne/--no-extensions`, `--skill`, `-ns/--no-skills`, `--prompt-template`, `-np/--no-prompt-templates`, `--theme`, `--use-theme`, `--no-themes`, `-nc/--no-context-files` | Local paths, trust, discovery and explicit override precedence | In progress: `-nc/--no-context-files` skips AGENTS/CLAUDE under process test; other listed flags absent |
