...
| Runtime | `--system-prompt`, `--append-system-prompt`, `--tui-mode`, `--verbose`, `-a/--approve`, `-na/--no-approve`, `--offline` | Per-invocation overrides, trust for project resources, no accidental network activity | `--approve` and `--no-approve` gate project system prompts; `--system-prompt` and repeatable `--append-system-prompt` resolve bounded UTF-8 files/literals and override discovery (local tests); `--no-context-files` skips AGENTS/CLAUDE loading (process fixture); remaining flags absent |
