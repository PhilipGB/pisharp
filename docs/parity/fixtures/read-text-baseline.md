...
`ReadTextConformanceTests` retains these **text-selection** expectations. The current pinned `read.ts` decodes its file buffer with `Buffer.toString("utf-8")`; the read source and tests under `packages/coding-agent/src/core/tools/` are unchanged between the current pin (`d5629e20489ccf770ed90b5a33941cb3b7ef24d0`) and fetched upstream main (`5fd446ca1843682e8da3fec4ceb71c42f56fbace`). A direct Node decoder probe returns `"﻿a"` for UTF-8 BOM bytes and `"��a\u0000"` for UTF-16LE BOM bytes, so BOMs are retained and other BOMs do not select another encoding.
