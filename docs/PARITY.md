...
| llama.cpp integration (`/llama`, provider, server management) | Equivalent | `extensions/llama/*` | The `llama.cpp` provider is a dynamic built-in: `LlamaServerApiKeyAuth` resolves the server from the stored credential env (`LLAMA_BASE_URL` persisted by login) or the ambient `LLAMA_BASE_URL`/`LLAMA_API_KEY`, defaulting to `http://127.0.0.1:8080` with the keyless `local` fallback; catalog refresh restores the last sync from models-store.json, then live-syncs from the router (`/models`, `/props` autoload) and filters with pinned `modelIsSelectable` (loaded/sleeping always, unloaded presets only under `models_autoload`); `toPiModel` maps `n_ctx`/`n_ctx_train` (128k fallback), image modality, and zero cost. `/login llama.cpp` prompts URL + optional key, verifies via `list()`, and persists `env.LLAMA_BASE_URL` (round-tripped through auth.json). The interactive `/llama` manages the server: model list with pinned descriptions, load (with replace-and-restore on failure), unload, and Hugging Face download (search, quantization aggregation with shard sums and Q4_K_M-first sort, gated-repo and 429/Retry-After handling, `HF_TOKEN`/`HF_TOKEN_PATH`/`HF_HOME`/XDG token discovery), followed by the online catalog refresh (15s timeout) | `LlamaProviderTests`, live router smoke |
