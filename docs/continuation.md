...
PiSharp uses a canonical C# session model and Microsoft Agent Framework for its provider/tool loop. Its session and RPC formats remain PiSharp-specific. The TUI has Markdig 1.3.2 AST parsing with separate terminal renderers, split transcript buffer/viewport/search state, shared display-cell wrapping, and a persistent idle alternate-screen compositor. The current local slice adds a reusable modal list host and an all/scoped model selector opened through Ctrl+L or `/model`. One Linux CI run passed on `ee11e9362`; its duplicate exposed a parallel listener teardown race in provider test fixtures. The worktree removes redundant double-stop cleanup and creates a fresh HttpListener per bind retry. Format/build and all 421 local tests pass; follow-up exact-head CI is pending. Markdown gaps include Pi's Unicode LaTeX layout, syntax highlighting and broader Marked dialect behavior. Session/settings pickers, editor/keybinding breadth, mouse/selection, themes, images and extension UI remain open.
