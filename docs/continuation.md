...
- With opt-in managed grep/find: format and warning-as-error build passed with zero warnings; deterministic tests passed **39/39**, including Git-ignore and fallback search. A `--continue` live run after migration and locking recalled `VIOLET-KITE`. Re-run after further edits.
