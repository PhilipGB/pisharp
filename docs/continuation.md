...
- At this checkpoint format verification and warning-as-error build passed with zero warnings; **63/63** deterministic tests passed, including a Linux pseudo-TTY interaction test and a process-kill checkpoint/recovery test. Opt-in `.97` JSON/RPC calls returned `PISHARP_JSON_OK` and `PISHARP_RPC_OK`. RPC now also exposes idle-only tree/entries/text/name operations, but still uses `format: pisharp` rather than Pi's wire contract. Full parity remains outstanding.

## Remaining priority

1. Audit pending checkpoint validity across all branches, corrupted and network-filesystem stores, provider switching and migration policy. Add process-kill tests at more boundaries; unknown side effects are not replayed. Implement shared execution events and improve terminal session controls.
2. Terminal now uses raw Linux VT input, bracketed paste and partial command/path completion, tested under PTY. Build real multi-row transcript/editor, resize, shortcuts, selectors, rendering, image display and asynchronous input while streaming; it still has only a **single-line viewport**, not a full TUI.
...
5. Project trust currently gates project system-prompt overrides with stored/interactive/CLI decisions; context AGENTS/CLAUDE remains loaded regardless of trust, like Pi. Extend that gate when implementing project settings, skills and executable extensions. `/trust` and `/reload` rebuild the agent. Keep committing tested slices. No whole feature is Verified.
