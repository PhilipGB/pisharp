...
PiSharp uses a canonical C# session model and Microsoft Agent Framework for its provider/tool loop. Its session and RPC formats remain PiSharp-specific. The TUI has Markdig 1.3.2 AST parsing with separate terminal renderers, split transcript buffer/viewport/search state, shared display-cell wrapping, and a persistent idle alternate-screen compositor. The shared modal list supports the all/scoped model picker through Ctrl+L or `/model`; the current working-tree slice adds a searchable project-scoped picker for `/resume` with direct ID/name resume retained and configurable `app.session.resume` (no default key). Cross-project session scope and the full selector controls remain open. Local format verification, warnings-as-errors build and all 423 tests pass for the session-picker working tree; exact-head CI is pending. The previous exact-head Linux CI checkpoint is `2b8673c9514efc17bb0789e79ab18d53dcc75ff6` / run 36132634038 (421 tests). Markdown gaps include Pi's Unicode LaTeX layout, syntax highlighting and broader Marked dialect behavior. Fork/settings pickers, editor/keybinding breadth, mouse/selection, themes, images and extension UI remain open.
