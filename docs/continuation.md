...
- At this checkpoint format verification and warning-as-error build passed with zero warnings; **58/58** deterministic tests passed, including a Linux pseudo-TTY interaction test and a process-kill checkpoint/recovery test. Opt-in `.97` JSON/RPC calls returned `PISHARP_JSON_OK` and `PISHARP_RPC_OK`. RPC now also exposes idle-only tree/entries/text/name operations, but still uses `format: pisharp` rather than Pi's wire contract. Full parity remains outstanding.
