...
- At previous checkpoint, format verification and warning-as-error build passed; 31/31 tests passed. Since then a normal-screen ANSI editor was added and manually smoke-tested in a Linux pseudo-terminal (`/session`, `/quit`), and 34/34 tests passed including deterministic editor state tests. Rerun format verification/build/tests before the next commit.
