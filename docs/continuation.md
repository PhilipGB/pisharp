...
- At this checkpoint, format verification and warning-as-error build passed with zero warnings; **46/46** deterministic tests passed. Opt-in `.97` JSON and RPC calls returned `PISHARP_JSON_OK` and `PISHARP_RPC_OK`. Experimental protocols are explicitly `format: pisharp`, not Pi wire-compatible. Full parity remains outstanding.
