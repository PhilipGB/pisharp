...
`--mode json <prompt>` emits LF-framed `pisharp` session and streaming lifecycle records described in [the protocol contract](docs/protocol.md). The same authoritative events drive the terminal, JSON and RPC adapters; a failed turn never emits successful completion. Transient provider failures are retried at most twice only when an attempt produced no content; partial text/tool-call attempts and tool execution are never automatically replayed. `--mode rpc` reads one JSON command per line on stdin, replies with correlated `response` records, and emits the same agent events without a session header. Supported RPC commands are `prompt` (nonempty text only), `get_state`, `get_messages`, `get_entries` (with cursor), `get_tree`, `get_last_assistant_text`, `set_session_name` (idle only), `get_commands`, `get_available_models`, `get_session_stats` (idle only; billing data unavailable), `export_html` (idle only; private file, no overwrite), `compact` (idle only), and `abort`. A second `prompt` during an active run queues a later turn before the run settles; this is not yet Pi's distinct steering/follow-up interface. Other commands reject explicitly; explicit steering/follow-up commands, images, provider controls and Pi-compatible payloads are **not implemented**. Both modes use the same MAF agent and canonical session store as print/terminal modes. Stdout is reserved for JSONL; consume it continuously. The `format: "pisharp"` marker means these event and session payloads are not upstream Pi wire-compatible.
